...
public class ModGuiConfigHUD extends GuiConfig {
    ...
    public void initGui() {
        super.initGui();
        int topLeftWidth = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.topleft")) +
                20, 100);

        int topRightWidth = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.topright")) +
                20, 100);

        int bottomLeftWidth = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.botleft")) +
                20, 100);

        int bottomRightWidth = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.botright")) +
                20, 100);

        int CenterTopWidth = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.centertop")) +
                20, 100);

        int CenterBottomWidth = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.centerbottom")) +
                20, 100);

        int CenterWidth = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.center")) +
                20, 100);

        int CenterLeft = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.centerleft")) +
                20, 100);

        int CenterRight = Math.max(this.mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.centerright")) +
                20, 100);
    ...
    public static class MapPosConfigEntry extends CategoryEntry {
        ...
        protected GuiScreen buildChildScreen() {
            ...
            String config = QualifiedName.substring(0, QualifiedName.indexOf(Configuration.CATEGORY_SPLITTER)).replace(Configuration.CATEGORY_SPLITTER, "");

            return new ModGuiConfigHUD(this.owningScreen, this.getConfigElement().getChildElements(), this.owningScreen.modID, null, this.configElement.requiresWorldRestart() ||
                    this.owningScreen.allRequireWorldRestart, this.configElement.requiresMcRestart() ||
                    this.owningScreen.allRequireMcRestart, this.owningScreen.title, config);
        }
    }
}
